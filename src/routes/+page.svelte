...
<div class="px-8 py-8">
	...
	<div class="flex items-start justify-between mb-8">
		...
		<div class="flex items-center gap-2 bg-white rounded-2xl px-4 py-2.5 shadow-card">
			...
			<div>
				<p class="text-xl font-black text-gray-900 leading-none">{mockUser.streak}</p>
	...
	<div class="grid grid-cols-3 gap-6 mb-6">
		...
		<div class="card col-span-2">
			<div class="flex items-center justify-between mb-4">
				...
				<a href="{base}/progress" class="text-xs text-brand-green font-semibold hover:underline">
					Full analytics →
				</a>
			</div>
	...
	<div class="grid grid-cols-3 gap-6">
				...
				{#each recommendations as rec}
					<div class="flex items-start gap-3 p-3 bg-gray-50 rounded-xl hover:bg-brand-green-light transition-colors">
						...
						<div class="flex-1 min-w-0">
							...
							<a href={rec.href} class="text-xs text-brand-green font-semibold mt-1 inline-block hover:underline">
								{rec.cta} →
							</a>
						</div>
					</div>
				{/each}
				<a href="{base}/library" class="block text-center text-xs text-gray-400 hover:text-brand-green transition-colors pt-1">
					Browse all content →
				</a>
			</div>
		</div>
		...
		<div class="card col-span-1">
			...
			<div class="flex flex-col items-center text-center">
				<div class="w-16 h-16 bg-gradient-to-br from-brand-green to-blue-400 rounded-full flex items-center justify-center mb-3">
	...
	{#if mockUser.plan === 'free'}
		<div class="mt-6 bg-gradient-to-r from-brand-pink to-orange-400 rounded-2xl p-5 flex items-center justify-between text-white">
			<div>
				<p class="font-black text-lg">You've used both free tests 🎯</p>
				<p class="text-sm opacity-90 mt-0.5">Unlock 13 more full practice tests + all Score Builder courses</p>
			</div>
			<button class="bg-white text-brand-pink font-black px-6 py-2.5 rounded-xl hover:bg-gray-50 transition-colors flex-shrink-0">
				Upgrade to Premium
			</button>
		</div>
	{/if}
