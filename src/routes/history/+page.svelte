<script lang="ts">
	...
	const allAvg  = $derived(secs.map(s => stats[s].avg));
	const genScore = $derived(allAvg.every(v => v !== null) ? roundHalf((allAvg as number[]).reduce((a, b) => a + b, 0) / 4) : null);
	const genBest  = $derived(secs.every(s => stats[s].best !== null) ? roundHalf(secs.reduce((a, s) => a + (stats[s].best as number), 0) / 4) : null);
	...
	const gaugeNA       = $derived(genScore === null || genScore === undefined);
	const gaugeColor    = $derived(gaugeNA ? '#ddd' : scoreColor(genScore as number));
	const gaugeFillAngle = $derived(gaugeNA ? SA : SA + ((genScore as number - 1) / 5) * TA);

	function toggleTest(num: string) {
		const key = sec + num;
		expanded[key] = expanded[key] !== false ? false : true;
	}
...
<div class="dashboard">
	...
	<div class="sticky-header">
		...
		<!-- Score Overview: Overall + 4 section cards in one row -->
		<div class="overview-row">
			<!-- Overall Score -->
			<div class="ov-card ov-overall">
				<span class="ov-label">Overall Avg</span>
				<div class="ov-big" style="color:{gaugeNA ? '#d0d5dd' : gaugeColor}">
					{gaugeNA ? '—' : fmtScore(genScore)}{#if !gaugeNA}<span class="ov-denom">/6</span>{/if}
				</div>
				{#if genBest !== null}
					<div class="ov-best">Best <b class="green">{fmtScore(genBest)}/6</b></div>
				{:else}
					<div class="ov-hint">Need all 4 sections</div>
				{/if}
			</div>
			...
			{#each secs as sc}
				...
				<button class="ov-card ov-sec" class:active={act} onclick={() => (sec = sc)}>
					...
					<div class="ov-score-row">
						...
						{#if s.avg !== null}<span class="ov-denom">/6</span><span class="ov-unit">avg</span>{/if}
					</div>
					<div class="ov-foot">
	...
	<div class="panel">
		...
		{:else if viewBy === 'test'}
			{#each Object.entries(groups) as [num, subs]}
				...
				<div>
					<button class="group-header" onclick={() => toggleTest(num)}>
						<span>
...
<style>
	* { box-sizing: border-box; margin: 0; padding: 0; }
	...
	/* ── Overview row: Overall + 4 section cards ── */
	.overview-row { display: grid; grid-template-columns: repeat(5, 1fr); gap: 8px; }
	...
	.ov-big { font-size: 30px; font-weight: 800; letter-spacing: -1.5px; line-height: 1; }
	.ov-denom { font-size: 14px; font-weight: 600; color: #bbb; }
	...
	.ov-name { font-size: 12px; font-weight: 600; color: #333; }
	.ov-score-row { display: flex; align-items: baseline; gap: 2px; margin-bottom: 6px; }
	.ov-val { font-size: 24px; font-weight: 800; letter-spacing: -1px; }
