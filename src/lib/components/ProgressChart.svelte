			...
			<div class="relative inline-block">
				<svg width={CW} height={CH} style="display:block;overflow:visible">
					...
					<g transform="translate({pL},{pT})">
						...
						<text x={iW + 5} y={trendData.yLast10 + 3.5} font-size="8.5" fill={trendData.color} font-family="DM Sans,sans-serif" font-weight="600">{latestAverage!.toFixed(1)}</text>

						<path d={trendData.area} fill="url(#tpgrd)" />
						<path d={trendData.d} fill="none" stroke={trendData.color} stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" />
						...
						{#each trendData.coords as c, i}
							<circle
								...
								stroke={trendData.color} stroke-width="2"
						...
						{#each trendData.coords as c, i}
							{#if i === 0 || i === trendData.coords.length - 1 || i === Math.floor((trendData.coords.length - 1) / 2)}
								<text x={c.x} y={iH + 17} text-anchor="middle" font-size="8.5" fill="#bbb" font-family="DM Sans,sans-serif">
				...
				{#if trendHovIdx !== null}
					...
					<div class="absolute pointer-events-none -translate-x-1/2 -translate-y-[calc(100%+10px)] bg-gray-900 text-white rounded-lg py-1.5 px-2.5 text-[11px] whitespace-nowrap flex flex-col items-center gap-px z-10 font-dmsans" style="left:{pL + c.x}px; top:{pT + c.y}px">
