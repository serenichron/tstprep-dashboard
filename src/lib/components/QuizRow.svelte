...
<div
  class="bg-white rounded-xl ring-1 ring-gray-100 shadow-[0_1px_3px_rgba(0,0,0,.04)] px-3.5 py-3 md:flex md:items-center md:px-5 md:py-2.5 md:gap-3 md:flex-nowrap transition-shadow hover:shadow-[0_3px_10px_rgba(0,0,0,.06)]"
>
  <!-- Header: test# + mode chip + (AI off) + date -->
  <div class="flex items-center gap-1.5 flex-wrap min-w-0 md:w-117.5 md:shrink-0 md:flex-nowrap">
    <span
      class="text-[11px] md:text-[10px] font-bold py-0.5 px-2 rounded-md bg-gray-100 text-gray-600 whitespace-nowrap"
      >Test #{quiz.test_number}</span
    >
    <span
      class="text-[10px] md:text-[9px] font-bold py-px px-1.75 rounded-full uppercase tracking-[.4px] whitespace-nowrap
    ...
    {#if aiOff}
      <span
        class="text-[10px] md:text-[9px] font-medium py-px px-1.75 rounded-full uppercase tracking-[.4px] bg-gray-100 text-gray-400 whitespace-nowrap"
        >AI off</span
      >
    {/if}
    <span class="text-[12px] md:text-xs text-gray-500 whitespace-nowrap ml-auto md:ml-0"
      >{date} <span class="text-gray-300">·</span> {time}</span
    >
  </div>
  ...
  <div class="flex gap-1 flex-wrap mt-2 md:mt-0 md:flex-1 md:min-w-0 md:order-3">
	...
	<div class="flex items-center gap-2 mt-3 md:mt-0 md:contents">
		<!-- Score bar + number -->
		<div class="flex items-center gap-2 flex-1 min-w-0 md:gap-1.5 md:flex-[0_0_120px] md:order-2">
			{#if !scored}
